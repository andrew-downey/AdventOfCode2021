    ...
    def process(self) -> int:
        if self.value == -1:
            if self.get_type() == 4:
                ...
                operation = self.get_type()
                sub_values = [packet.process() for packet in self.sub_packets]
                if operation == 0:
                    self.value = sum(sub_values)
                elif operation == 1:
                    self.value = reduce(lambda x, y: x * y, sub_values)
                elif operation == 2:
...
def main():
    ...
    print("")
    print("Total value of packets: {0}".format(
        sum([packet.process() for packet in packets])))
