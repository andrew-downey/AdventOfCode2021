...
def explode(elements):
    open_brackets = 0
    for opening_bracket_index in range(len(elements)):
        if open_brackets == 4 and elements[opening_bracket_index] == '[':
            ...
            elements = elements[:opening_bracket_index] + \
                [0] + elements[opening_bracket_index + 4:]
            return True, elements
        elif elements[opening_bracket_index] == '[':
            open_brackets += 1
        elif elements[opening_bracket_index] == ']':
            open_brackets -= 1
...
def reduce(elements: List) -> List:
    has_reduced = True
    while has_reduced:
        has_reduced, elements = explode(elements)
        if not has_reduced:
...
def main():
    ...
    for element_a in elements:
        for element_b in elements:
            ...
            current_magnitude = magnitude(result)[0]
            max_magnitude = max(max_magnitude, current_magnitude)

    print("")
    print("Result: {0}".format(max_magnitude))
