...
def main():
    ...
    x_velocity_range = (2, grid_x[1])
    y_velocity_range = (-2 * abs(grid_y[0]),
                        2 * abs(grid_y[0]))
    print(x_velocity_range)
    print(y_velocity_range)
    progress = ProgressBar((x_velocity_range[1] - x_velocity_range[0]) * (
        y_velocity_range[1] - y_velocity_range[0])).start()

    for x_velocity in range(x_velocity_range[0], x_velocity_range[1]):
        for y_velocity in range(y_velocity_range[0], y_velocity_range[1]):
            progress.update(x_velocity * abs(y_velocity))
    ...
    y_maximums = {(probe.initial_x, probe.initial_y)
                   : probe.y_max for probe in successful_probes}
    y_maximum = sorted(y_maximums.items(), key=lambda x: x[1], reverse=True)[0]
    print("Successful probes: {0}".format(len(successful_probes)))
