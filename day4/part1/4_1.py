...
def main():
    ...
    for number in bingo_callouts.split(","):
        ...
        for i in range(len(bingo_boards)):
            ...
            if(current_board.stamp(number).check()):
                winner = i
                winning_number = number
                winner_score = current_board.score(winning_number)
                break
        if(winner != None):
            break
    ...
    print("winning board {0}".format(winner + 1))
    print("winning callout {0}".format(winning_number))
    print("winner board score {0}".format(
        int(winner_score / int(winning_number))))
    print("winner total score {0}".format(winner_score))
    bingo_boards[winner].debug()
